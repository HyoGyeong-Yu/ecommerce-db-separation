#!/bin/bash
# ============================================================================
# 시나리오 4: EC2 접근 불가 (인스턴스 중지)
# 재현: 앱 서버 EC2를 중지(stop) → 서비스 접근 불가
# 포인트: StatusCheckFailed 알람 발생 → CloudWatch 감지 → 복구
# ============================================================================
source ./config.sh
...
aws cloudwatch describe-alarms \
  --alarm-names "${PROJECT_NAME}-ec2-status-check-failed" \
  --region "$REGION" \
...
log_warning "   2. CloudWatch — ec2-status-check-failed 알람이 ALARM(빨강)으로"
log_warning "   3. 이메일 — SNS 알람 메일 도착"

echo ""
log_info "알람 상태 다시 확인 (1~2분 후 실행 권장):"
log_info "aws cloudwatch describe-alarms --alarm-names ${PROJECT_NAME}-ec2-status-check-failed --region $REGION --query 'MetricAlarms[0].StateValue'"
